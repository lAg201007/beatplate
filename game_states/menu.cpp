...
void MainMenu::update(sf::Time dt) {
    ...
    whiteMaskShader.setUniform("WhiteMultiplier", TitleWhiteMaskTween->getValue());

    if (TitleTween) {
        TitleTween->update(dt.asSeconds());
        TitleWhiteMaskTween->update(dt.asSeconds()); 

        if (!TitleTween->isActive()) {
            TitleTween->reset();
            TitleTween->play();
            TitleWhiteMaskTween->reset();
            TitleWhiteMaskTween->play();
            AudioManager::getInstance().playTemporarySound(circle_beat_buffer, 5.f);
        }
    }
    ...
    if (Title.DetectButtonClick(mWindow) && !isPendingStateChange) {
        start_sound.play();
    ...
    if (isPendingTweenChange && TitleWhiteMaskTween && !TitleWhiteMaskTween->isActive()) {
        TitleWhiteMaskTween = std::make_unique<ValueTween>(TitleWhiteMaskTween->getValue(), 0.f, 0.2f);
        TitleWhiteMaskTween->play();
        
        isPendingStateChange = true;
        isPendingTweenChange = false;
    }

    if (isPendingStateChange && TitleWhiteMaskTween && !TitleWhiteMaskTween->isActive()) {
        TitleWhiteMaskTween = std::make_unique<ValueTween>(TitleBeatWhiteMultiplier, 0.f, 0.5f);
        ...
        if (!musicPath.empty() && musicPath != lastCheckedMusicPath) {
            ...
            if (jsonFile) {
                ...
                if (bpm != ActualMusicBpm) {
                    ...
                    TitleWhiteMaskTween = std::make_unique<ValueTween>(TitleBeatWhiteMultiplier, 0.f, tweenTime);
                    TitleWhiteMaskTween->play();
                }
            }
        }
...
void MainMenu::render() {  
    ...
    mWindow.draw(*Title.sprite, &whiteMaskShader);
